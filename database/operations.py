...
# RadheFoundation Application - Database Operations
...
from database.models import (
    ...
    Task, TaskComment, AuditLog, Message, TimeOff, BillingRequest, BillingRequestAttachment,
...
def delete_employee_complete(db: Session, emp_id: int) -> dict:
    """
    Complete employee deletion with all cascades.
    Deletes: Employee, User, Attendance, Leaves, Tasks, Messages, Notifications, etc.
    Returns: {'success': bool, 'message': str, 'deleted_counts': dict}
    """
    deleted_counts = {
        'employees': 0, 'users': 0, 'attendance': 0, 'leaves': 0, 'tasks': 0,
        'emails': 0,
        'org_hierarchy': 0,
        'chat_members': 0
    }
    ...
    try:
        ...
        # 1. Attendance
        # 0a. BILLING REQUESTS + ATTACHMENTS (FK cascade fix)
        billing_request_ids = [r[0] for r in db.query(BillingRequest.id).filter(
            BillingRequest.employee_id == emp_id
        ).all()]
        if billing_request_ids:
            # Delete attachments FIRST (child table)
            db.query(BillingRequestAttachment).filter(
                BillingRequestAttachment.billing_request_id.in_(
                    billing_request_ids)
            ).delete()
            # THEN billing requests
            billing_requests = db.query(BillingRequest).filter(
                BillingRequest.id.in_(billing_request_ids)
            ).delete()
            deleted_counts['billing_requests'] = billing_requests

        # 1. Attendance
        att_count = db.query(Attendance).filter(
            Attendance.employee_id == emp_id).delete()
        deleted_counts['attendance'] = att_count

        # 2. Leaves
        leave_count = db.query(LeaveRequest).filter(
            LeaveRequest.employee_id == emp_id).delete()
        lb_count = db.query(LeaveBalance).filter(
            LeaveBalance.employee_id == emp_id).delete()
        deleted_counts['leaves'] = leave_count + lb_count

        # 2.5 Task Comments (before Tasks - FK constraint)
        task_ids = [t[0] for t in db.query(Task.id).filter(
            (Task.assigned_to_id == emp_id) | (Task.created_by_id == emp_id)
        ).all()]
        if task_ids:
            comment_count = db.query(TaskComment).filter(
                TaskComment.task_id.in_(task_ids)
            ).delete()
            deleted_counts['task_comments'] = comment_count

        # 3. Tasks (assigned/created)
        task_assigned = db.query(Task).filter(
            Task.assigned_to_id == emp_id).delete()
        task_created = db.query(Task).filter(
            Task.created_by_id == emp_id).delete()
        deleted_counts['tasks'] = task_assigned + task_created

        # 4. Messages (user_id)
        if user_id:
            msg_sent = db.query(ChatMessage).filter(
                ChatMessage.sender_id == user_id).delete()
            msg_rec = db.query(ChatMessage).filter(
                ChatMessage.receiver_id == user_id).delete()
            deleted_counts['messages'] = msg_sent + msg_rec

        # 5. Notifications
        if user_id:
            from database.models import AppNotification
            notif_count = db.query(AppNotification).filter(
                AppNotification.user_id == user_id).delete()
            deleted_counts['notifications'] = notif_count

        # 6. Call logs
        if user_id:
            call_count = db.query(CallLog).filter(
                (CallLog.caller_id == user_id) | (
                    CallLog.receiver_id == user_id)
            ).delete()
            deleted_counts['call_logs'] = call_count

        # 7. Permissions
        if user_id:
            perm_count = db.query(UserPermission).filter(
                UserPermission.user_id == user_id).delete()
            deleted_counts['permissions'] = perm_count

        # 8. Documents
        if user_id:
            doc_count = db.query(Document).filter(
                Document.uploaded_by == user_id).delete()
            deleted_counts['documents'] = doc_count

# 9. Emails sent by user (FIXED: Clean ALL EmailRecipient records for sender's emails + user_id references)
        if user_id:
            logger.info(f"🧹 FIXED: Full cleanup for user_id={user_id}")

            # MEETINGS - CRASH FIX #1
            meeting_part_count = db.query(MeetingParticipant).filter(
                MeetingParticipant.user_id == user_id
            ).delete()
            deleted_counts['meeting_participants'] = meeting_part_count

            # Emails - keep original + add logging
            rec_as_recipient = db.query(EmailRecipient).filter(
                EmailRecipient.recipient_id == user_id
            ).delete()
            deleted_counts['email_recipients_as_recipient'] = rec_as_recipient

            # Get sent email IDs (where this user was sender)
            sent_email_query = db.query(EmailMessage.id).filter(
                EmailMessage.sender_id == user_id
            )
            email_ids = [eid[0] for eid in sent_email_query.all()]

            # Delete remaining EmailRecipient for sent emails (in case any missed)
            if email_ids:
                rec_as_sender_emails = db.query(EmailRecipient).filter(
                    EmailRecipient.email_id.in_(email_ids)
                ).delete()
                deleted_counts['email_recipients_sent_emails'] = rec_as_sender_emails

                # Delete EmailMessages sent by this user
                msg_count = db.query(EmailMessage).filter(
                    EmailMessage.id.in_(email_ids)
                ).delete()
                deleted_counts['sent_emails'] = msg_count
            else:
                deleted_counts['sent_emails'] = 0

            total_email_recipients = rec_as_recipient + rec_as_sender_emails
            deleted_counts['email_recipients'] = total_email_recipients
            logger.info(
                f"Email cleanup complete: recipients={total_email_recipients}, emails={deleted_counts['sent_emails']}")

        # 8.5 Team Memberships (CRITICAL FIX - NOT NULL VIOLATION)
        from database.models import TeamMember, ProjectMember, OrgHierarchy
        ...
        logger.info(
            f"Project cleanup complete: members={project_member_count}")

        # 8.7 Org Hierarchy (safer cleanup both directions)
        org_count = db.query(OrgHierarchy).filter(
            or_(OrgHierarchy.employee_id == emp_id,
                OrgHierarchy.reports_to_id == emp_id)
        ).delete(synchronize_session=False)
        deleted_counts['org_hierarchy'] = org_count
        logger.info(f"Org hierarchy cleanup complete: records={org_count}")

        # 8.8 Chat Group Memberships (NEW: FK violation fix)
        chat_member_count = db.query(ChatGroupMember).filter(
            ChatGroupMember.user_id == user_id).delete(synchronize_session=False)
        deleted_counts['chat_members'] = chat_member_count
        logger.info(
            f"Chat group cleanup complete: members={chat_member_count}")

        # 10. Delete Employee & User
        db.delete(employee)
        ...
        # Audit log
        create_audit_log(
            db, 1, f"COMPLETE_EMPLOYEE_DELETE_{emp_id}", f"Deleted employee {emp_id} + {sum(deleted_counts.values())} records")

        db.commit()

        # Cache invalidation
        _invalidate_cache('employee')
        _invalidate_cache('dashboard')

        total = sum(deleted_counts.values())
        return {'success': True, 'message': f'Employee & {total} records deleted', 'deleted_counts': deleted_counts}

    except Exception as e:
        db.rollback()
        logger.error(f"Delete employee {emp_id} failed: {e}")
        return {'success': False, 'message': f'Failed: {str(e)}', 'deleted_counts': deleted_counts}
...
def create_meeting(db: Session, title: str, organizer_id: int,
    ...
    meeting = Meeting(
        ...
        meeting_link=f"https://RadheFoundation.local/meeting/{organizer_id}_{int(start_time.timestamp())}"
    )
    db.add(meeting)
    db.flush()
...
def get_internal_email(username: str) -> str:
    ...
    return f"{username}@RadheFoundation.local"


def get_username_from_email(email: str) -> str:
    """Extract username from internal email"""
    if email.endswith("@RadheFoundation.local"):
        return email.replace("@RadheFoundation.local", "")
    return email
