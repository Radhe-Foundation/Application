"""
RadheFoundation - Database Indexes for Performance (Fixed Version)
...
from database.session_manager import get_engine
...
def create_index_safe(index_name, table_name, columns):
    """Create an index safely outside transaction blocks"""
    engine = get_engine()
    try:
...
def create_composite_index_safe(index_name, table_name, columns):
    """Create a composite index safely"""
    return create_index_safe(index_name, table_name, columns)


def create_performance_indexes():
    """Create all single-column indexes for better performance"""
    ...
    indexes = [
        ...
        ("idx_timesheets_employee_id", "timesheets", "employee_id"),
        ("idx_timesheets_week_start", "timesheets", "week_start"),
        ("idx_time_entries_timesheet_id", "time_entries", "timesheet_id"),
        ("idx_time_entries_project_id", "time_entries", "project_id"),
        ...
        # Billing indexes
        ("idx_billing_requests_employee_id", "billing_requests", "employee_id"),
        ("idx_billing_requests_status", "billing_requests", "status"),
        ("idx_billing_requests_request_number",
         "billing_requests", "request_number"),
    ]

    for index_name, table_name, column_name in indexes:
        create_index_safe(index_name, table_name, column_name)
...
def create_composite_indexes():
    ...
    composite_indexes = [
        ...
        ("idx_audit_logs_user_time", "audit_logs", "user_id, created_at"),

        # Leave balances - employee/year
        ("idx_leave_balances_employee_year", "leave_balances", "employee_id, year"),

        # Timesheets - employee/week
        ("idx_timesheets_employee_week", "timesheets", "employee_id, week_start"),
    ]

    for index_name, table_name, columns in composite_indexes:
        create_composite_index_safe(index_name, table_name, columns)
